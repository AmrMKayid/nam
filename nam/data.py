...
DATA_PATH = '/home/amrmkayid/nam/data'
...
def load_sklearn_housing_data():
    """Loads the sklearn housing dataset."""
...
def transform_data(df):
    ...
    for index, is_cat in enumerate(is_categorical):
        ...
        if is_cat:
            new_column_names += ['{}: {}'.format(col_name, val) for val in set(df[col_name])]
        else:
...
def load_dataset(dataset_name):
    ...
    elif dataset_name == 'sklearn_housing':
        dataset = load_sklearn_housing_data()
...
def get_train_test_fold(data_x, data_y, fold_num, num_folds, stratified=False, random_state=42):
    ...
    """
    if stratified:
        stratified_k_fold = StratifiedKFold(n_splits=num_folds, shuffle=True, random_state=random_state)
    else:
        stratified_k_fold = KFold(n_splits=num_folds, shuffle=True, random_state=random_state)
...
def split_training_dataset(data_x, data_y, n_splits, stratified=False, test_size=0.125, random_state=1337):
    """Yields a generator that randomly splits data into (train, validation)
    set.
    ...
    """
    if stratified:
        stratified_shuffle_split = StratifiedShuffleSplit(n_splits=n_splits,
                                                          test_size=test_size,
                                                          random_state=random_state)
    else:
        stratified_shuffle_split = ShuffleSplit(n_splits=n_splits, test_size=test_size, random_state=random_state)
...
def create_test_train_fold(dataset: Union[str, Tuple[pd.DataFrame, pd.DataFrame]],
                           id_fold: int,
                           n_folds: int,
                           n_splits: int,
    ...
    (x_train_all, y_train_all), test_dataset = get_train_test_fold(data_x,
                                                                   data_y,
                                                                   fold_num=id_fold,
                                                                   num_folds=n_folds,
                                                                   stratified=regression)
    data_gen = split_training_dataset(x_train_all, y_train_all, n_splits, stratified=regression)
